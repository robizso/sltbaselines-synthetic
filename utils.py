...
def save_on_master(*args, **kwargs):
    if is_main_process():
        import os, time
        path = args[1] if len(args) > 1 else kwargs.get('f', 'unknown')
        try:
            torch.save(*args, **kwargs)
            if not os.path.exists(str(path)) or os.path.getsize(str(path)) < 1024:
                raise RuntimeError(f"Checkpoint file missing or too small after save: {path}")
            print(f"Checkpoint saved: {path} ({os.path.getsize(str(path))/1024**2:.0f} MB)", flush=True)
        except Exception as e:
            print(f"ERROR saving checkpoint {path}: {e}", flush=True)
            raise
...
def init_distributed_mode(args):
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        print("RANK and WORLD_SIZE:", args.rank, args.world_size)
...
def read_lmdb_folder(lmdb_path, folder_name=None):
    ...
    with env.begin(write=False) as txn:
        images_data = txn.get(key="data".encode('ascii'))
