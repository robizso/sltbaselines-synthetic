...
# *user-defined
from dataloader.datasets_synthetic import S2T_Dataset
...
def main(args, config):
    ...
    if args.model_type in ["flallm", "c2rl"] and args.finetune and not args.eval:
        ...
        # *replace the word embedding (only if vocab matches model)
        from transformers import MBartConfig, MBartForConditionalGeneration
        _cfg = MBartConfig.from_pretrained(config['model']['transformer'])
        if _cfg.vocab_size == model.mbart.config.vocab_size:
            _transformer_model = MBartForConditionalGeneration.from_pretrained(config['model']['transformer'])
            model_dict = _transformer_model.state_dict()
            for k, v in model_dict.items():
                if any(item in k for item in ['decoder.embed_tokens.weight', 'decoder.embed_positions.weight']):
                    k = 'mbart.' + k
                    new_state_dict[k] = v
        else:
            print(f'Skipping decoder embedding replacement: transformer vocab {_cfg.vocab_size} != model vocab {model.mbart.config.vocab_size}')
    ...
    if test_on_last_epoch and args.output_dir:
        checkpoint = torch.load(args.output_dir+'/best_checkpoint.pth', map_location='cpu', weights_only=False)
        model.load_state_dict(checkpoint['model'], strict=True)
...
def evaluate(args, dev_dataloader, model, tokenizer, criterion,  config, UNK_IDX, SPECIAL_SYMBOLS, PAD_IDX, device, split="dev"):
    ...
    # clean BLEU: strip de_DE language tag, append dot for phoenix
    _lang_tag = config['data'].get('tgt_lang', 'de_DE')
    tgt_pres = [' '.join(s.replace(_lang_tag, '').split()) for s in tgt_pres]
    tgt_refs = [' '.join(s.replace(_lang_tag, '').split()) for s in tgt_refs]
